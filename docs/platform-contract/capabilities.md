...
| `BFS-CAP-FUSE-MUTATION` | POSIX writable namespace and data operations | `src/fuse/`, `src/core/` | Prohibited | M6 acceptance only |
| `BFS-CAP-POSIX-OPEN-UNLINK` | Open file survives unlink and restart safely | `src/core/`, future format | Unavailable in v2 | v3 design and crash matrix |
| `BFS-CAP-POSIX-RENAME-REPLACE` | Atomic replacement rename with crash safety | `src/core/`, future format | Unavailable in v2 | v3 design and crash matrix |
...
`BFS-CAP-FUSE-MUTATION` must remain absent from the M5 binary and manifest.
The capability test must verify that every advertised operation has a matching
mount test and that every rejected mutation returns `EROFS`.
