...
class AmigaRunnerTests(unittest.TestCase):
    ...
    def test_inventory_filter_is_strict_and_selective(self):
        ...
        inventory = RUNNER.test_inventory(ROOT / "tools/bfs-test-cases.def", "exnext_37+fill_08")
        self.assertEqual(inventory, ["exnext_37", "fill_08"])
        with self.assertRaisesRegex(ValueError, "invalid test filter"):
            RUNNER.test_inventory(ROOT / "tools/bfs-test-cases.def", "fill 08")
        with self.assertRaisesRegex(ValueError, "invalid test filter"):
            RUNNER.test_inventory(ROOT / "tools/bfs-test-cases.def", "fill_08+")
